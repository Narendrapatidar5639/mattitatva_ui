...
import { useNavigate } from "react-router"; // <-- 1. react-router से useNavigate इम्पोर्ट किया
import {
...
export function NearbyShopPage(props: any) {
  const routerNavigate = useNavigate(); // <-- 2. नेविगेशन के लिए हुक इनिशियलाइज़ किया
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {NEARBY_SHOPS.map((shop, i) => {
          ...
          return (
            <div
              key={shop.id}
              onClick={() => routerNavigate(`/nearby/${shop.id}`)}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white flex-shrink-0 transition-transform group-hover:scale-105" style={{ background: shade }}>
                    <MapPin size={16} />
                  </div>
                  <div>
                    {/* जब यूजर होवर करे तो नाम थोड़ा हाईलाइट होगा */}
                    <h3 className="text-sm font-bold text-gray-800 group-hover:text-black transition-colors">{shop.name}</h3>
              ...
              <p className="text-xs text-gray-500 mb-1 pl-1">{shop.address}</p>
              <p className="text-xs text-gray-400 mb-3 flex items-center gap-1 pl-1">
                <Phone size={10} /> {shop.phone}
              </p>
              
              <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                {/* e.stopPropagation() लगाया है ताकि नीचे के बटन दबाने पर पैरेंट कार्ड का पेज चेंज वाला क्लिक न चल जाए */}
                <button 
                  onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shop.name + " " + shop.address)}`, '_blank')}
                  className="flex-1 py-1.5 text-white text-[11px] font-bold rounded-xl active:scale-95 transition-transform" 
                ...
                <a 
                  href={`tel:${shop.phone}`}
                  className="flex-1 py-1.5 border text-[11px] font-bold rounded-xl text-center active:scale-95 transition-transform" 
